...
exports.demandesRouter.get("/", (0, role_1.allowRoles)("agent", "responsable", "admin"), async (req, res) => {
    if (!req.tenantId || !req.user)
        return res.status(400).json({ message: "Tenant requis" });
    const where = {
        etablissementId: req.tenantId,
    ...
    const demandes = await prisma_1.prisma.demande.findMany({ where, include: { items: true } });
    res.json(demandes);
});
exports.demandesRouter.get("/:id", (0, role_1.allowRoles)("agent", "responsable", "admin"), async (req, res) => {
