...
export function SupplierOrdersSection() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) =>
        setOrdersError(err instanceof Error ? err.message : "Impossible de charger les commandes fournisseurs"),
      )
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) =>
        setSuppliersError(err instanceof Error ? err.message : "Impossible de charger les fournisseurs"),
      )
  ...
  const selectedItems = useMemo(
    () =>
      articles
        .map((article) => ({ articleId: article.id, quantite: quantities[article.id] ?? 0, article }))
  ...
  const handleAddSupplier = (event?: React.FormEvent) => {
    ...
    const existing = suppliers.find(
      (s) => s.nom === supplierName.trim() && s.adresse === supplierAddress.trim(),
    );
    if (existing) {
      setSelectedSupplierId(existing.id);
      setSubmitMessage("Fournisseur déjà enregistré, sélection mis à jour.");
      return;
    }
    ...
    api
      ...
      .then((created) => {
        const enriched: SupplierProfile = { ...profile, id: created.id, adresse: created.adresse || "" };
        setSuppliers((prev) => [enriched, ...prev]);
        setSelectedSupplierId(created.id);
        setSubmitMessage("Fournisseur enregistré en base.");
      })
      .catch((err) => {
        setSubmitMessage(err instanceof Error ? err.message : "Impossible d'enregistrer le fournisseur");
      });
  };
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      const payload: {
        ...
        fournisseur:
          supplierAddress.trim() && !selectedSupplierId
            ...
            : supplierName.trim(),
        items: selectedItems.map((item) => ({ articleId: item.articleId, quantite: item.quantite })),
      };
      ...
      setSubmitMessage(err instanceof Error ? err.message : "Impossible de créer la commande fournisseur");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleGeneratePdf = () => {
    ...
    const safeName = supplierName.trim() || "fournisseur";

    doc.setFontSize(16);
    doc.text("Bon de commande fournisseur", 105, 18, { align: "center" });
    doc.setFontSize(11);
    doc.text(`Date : ${formattedDate}`, 14, 28);
    doc.text(`Fournisseur : ${safeName}`, 14, 35);
    if (supplierAddress.trim()) {
      doc.text(`Adresse : ${supplierAddress.trim()}`, 14, 42);
    }

    doc.setFontSize(12);
    doc.text("Articles", 14, 55);
    ...
    selectedItems.forEach((item, index) => {
      ...
      doc.setFontSize(11);
      doc.text(`${index + 1}. ${product.nom}`, 14, y);
      doc.setFontSize(10);
      doc.text(`Référence: ${product.referenceFournisseur ?? "—"}`, 14, y + 6);
      doc.text(`Quantité: ${item.quantite}`, 14, y + 12);
      doc.line(14, y + 14, 196, y + 14);
      y += 22;
    });

    doc.save(`bon-commande-${safeName.replace(/\\s+/g, "-").toLowerCase()}.pdf`);
    setSubmitMessage("Bon de commande PDF généré.");
  };
  ...
  const handleMarkReceived = async (orderId: string) => {
    try {
      ...
      setOrdersError(err instanceof Error ? err.message : "Impossible de valider la commande");
    }
  };
  ...
  const handleSaveOrderEdit = async (order: SupplierOrder) => {
    ...
    try {
      ...
      setOrdersError(err instanceof Error ? err.message : "Impossible de modifier la commande");
    }
  };

  const inProgressOrders = orders.filter((order) => order.statut === "en_cours");
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        description="Sélectionnez les articles du stock, renseignez le fournisseur, générez un PDF et créez la commande auprès du fournisseur."
      />

      <Card className="border-slate-200">
        <CardHeader
          title="Fournisseur"
          subtitle="Nom et adresse figurent sur le bon de commande"
          action={
            isSuperAdmin ? (
              <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700">
                Superadmin : créer via un rôle établissement
              </span>
            ) : null
          }
        />
        <div className="space-y-2.5 px-4 pb-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center gap-1.5 text-sm text-slate-700">
              ...
              <select
                ...
                onChange={(event) => handleSelectSupplier(event.target.value)}
              >
            ...
            {suppliersLoading ? (
              ...
              <span className="text-xs text-rose-600">{suppliersError}</span>
            ) : null}
            <button
              type="button"
              className="rounded-full bg-slate-900 px-3.5 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-slate-800"
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              className="rounded-full bg-slate-900 px-3.5 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-slate-800 disabled:opacity-50"
            ...
            <span className="text-xs text-slate-500">
              Les commandes sont disponibles pour les rôles admin / responsable reliés à un établissement.
            </span>
          </div>
      ...
      <Card className="border-slate-200">
        <CardHeader title="Commandes fournisseurs" subtitle="En cours et historique" />
        {isSuperAdmin ? (
          <p className="px-4 pb-4 text-sm text-slate-500">
            Connectez-vous avec un rôle établissement (admin ou responsable) pour consulter les commandes.
          </p>
        ) : ordersLoading ? (
          <p className="px-4 pb-4 text-sm text-slate-500">Chargement des commandes...</p>
        ) : ordersError ? (
          ...
          <div className="grid gap-4 px-4 pb-4 md:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-white/70 p-3 shadow-sm">
              <div className="mb-2 flex items-center justify-between">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.18em] text-slate-500">En attente</p>
                  <p className="text-sm font-semibold text-slate-900">Commandes en cours</p>
                </div>
              ...
              {inProgressOrders.length === 0 ? (
                <p className="text-sm text-slate-500">Aucune commande en cours.</p>
              ) : (
                      ...
                      <div className="space-y-2">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div className="space-y-1">
                            <p className="font-semibold text-slate-900">{order.fournisseur}</p>
                        ...
                        {editingOrderId === order.id ? (
                          <div className="space-y-1 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
                            {order.items.map((item) => (
                              <div key={item.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-2 text-xs">
                                <span className="text-slate-700">
                                  {articles.find((a) => a.id === item.articleId)?.nom ?? "Article"}
                                </span>
                                <input
                                  type="number"
                                  min={1}
                                  className="w-16 rounded-full border border-slate-200 px-2 py-1 text-center focus:border-emerald-500/70 focus:outline-none"
                                  value={editingItems[item.articleId] ?? item.quantite}
                                  onChange={(event) =>
                        ...
                        >
                          {expandedOrders.includes(order.id) ? "Masquer le détail" : "Voir le détail"}
                        </button>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white/70 p-3 shadow-sm">
              <div className="mb-2 flex items-center justify-between">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.18em] text-slate-500">Historique</p>
                  <p className="text-sm font-semibold text-slate-900">Commandes reçues</p>
                </div>
                <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-[11px] font-semibold text-slate-700">
              ...
              {receivedOrders.length === 0 ? (
                <p className="text-sm text-slate-500">Aucune commande reçue.</p>
              ) : (
                <ul className="space-y-2 text-sm">
                    ...
                    >
                      <div className="space-y-2">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-slate-500">
                            {order.items.length} article(s)
                            {order.updatedAt ? ` • Reçue le ${new Date(order.updatedAt).toLocaleDateString()}` : ""}
                          </p>
                        </div>
      ...
      <Card>
        ...
        />
        {loading ? (
          <p className="px-4 pb-4 text-sm text-slate-500">Chargement des articles...</p>
        ) : error ? (
          <p className="px-4 pb-4 text-sm text-rose-600">{error}</p>
        ) : filteredArticles.length === 0 ? (
          <p className="px-4 pb-4 text-sm text-slate-500">Aucun article trouvé.</p>
        ) : (
                    ...
                    <div className="space-y-1">
                      ...
                      {inAlert ? (
                        <span className="inline-flex w-fit items-center rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-amber-800 ring-1 ring-amber-200">
                          Niveau d&apos;alerte (stock: {article.quantite}, seuil: {article.seuilAlerte})
                        </span>
                      ) : null}
                    </div>
                    <div className="grid grid-cols-3 items-center gap-2 sm:grid-cols-4">
                      <span className="text-center text-slate-600">{article.referenceFournisseur ?? "—"}</span>
                      <span className="text-center font-semibold text-slate-900">{article.quantite}</span>
                      <input
                        type="number"
                        min={0}
                        ...
                        onChange={(event) =>
                          setQuantities((prev) => ({
                            ...prev,
                            [article.id]: Math.max(0, Number(event.target.value)),
                          }))
                        }
                      />
                      <span className="hidden text-right text-slate-600 sm:block">{article.seuilAlerte}</span>
                    </div>
                  </div>
                );
              })}
